...
- **Shareable links.** The view root and the 2D/3D mode live in the URL (`use-view-url.ts`), so a copied link opens the tree from the same person in the same view. Defaults are left out of it, the selection is not part of it, and changes replace the history entry rather than adding one.
