...
- **Birth date:** an optional research aid in a fixed, machine-readable shape, so tools and AI can read and edit it reliably instead of parsing notes. It is a partial ISO date: a year, a year and month, or a full date (shaped like `1931`, `1931-06`, `1931-06-16`), as precise as the sources allow. `treeProblems` rejects anything else, including impossible dates. Only the year appears on the cards, as "b. 1931" on the same small line as "née …", so the card keeps its height; the full date shows only in the edit panel. There is deliberately no death date field: that would be a deceased flag by another name, so death dates stay in notes.
