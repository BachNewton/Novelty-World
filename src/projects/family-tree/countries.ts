// The curated heritage countries. Each needs a designed look (its flag), so
// the list lives in code rather than accepting any name. Adding a country is
// one entry here.
//
// Flag fills are each flag's official colors: they are data about the
// country, not UI theme colors, so they live here as literal values rather
// than as design-system tokens.
...
// Codes are ISO 3166-1 alpha-2, or ISO 3166-2 for the nations of Great Britain.
...
// A heritage entry the owner can make when a line's origin isn't known.
export const UNKNOWN_HERITAGE = "unknown";

export type HeritageCode = CountryCode | typeof UNKNOWN_HERITAGE;

export function isCountryCode(value: string): value is CountryCode {
  return Object.hasOwn(COUNTRIES, value);
}

export function isHeritageCode(value: unknown): value is HeritageCode {
  return typeof value === "string" && (value === UNKNOWN_HERITAGE || isCountryCode(value));
}

// Every country, alphabetical by name, for pickers.
export const COUNTRY_CODES_BY_NAME: readonly CountryCode[] = (
