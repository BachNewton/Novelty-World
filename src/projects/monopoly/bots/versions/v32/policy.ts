// ===========================================================================
// v32 SNAPSHOT — fork of the champion v29 (see EVOLUTION.md). VERBATIM v29 except
// for ONE added mechanism, MORTGAGE-TO-FUND a DISTRESS COMPLETION (roadmap #2's
// open sweetener half, hard-gated to the proven distress shape):
//
//   v29 returns NULL on a +EV distress-discounted completion it can't fund in
//   CASH but COULD fund by mortgaging an idle back-burner lot (`sweetenForAll`
//   bails when `myCash < total`). A distressed rival selling its completer cheap
//   is the fleeting, asymmetric, UNDERPRICED opportunity v28/v29 win on — so when
//   the bot is one mortgage away from seizing it, it should pre-raise the cash.
//
// The trade flow has NO in-trade raise phase (unlike the buy path's `raising-cash`),
// so the funding is CROSS-TURN: at pre-roll the policy arms a mortgage-only `manage`
// to raise the shortfall (`distressRaisePlan`), then on a LATER turn `proposeBestTrade`
// proposes the now-cash-fundable completion via Offer B. The detector
// (`distressCompletionNeedingCash`) HARD-GATES to a genuinely distressed seller, a
// +EV-and-all-accept completion, blocked SOLELY by cash and within mortgage reach —
// NOT a general mortgage-to-fund (which would be v4's washed plain tempo). It
// self-limits to ONE raise: once mortgaged, cash covers the sweetener and the
// detector returns null (so it won't re-arm and bleed interest in a loop).
//
// Everything else — the dispatcher, the desperation mechanism, v17's reserve, v14's
// phantom-denial fix — is v29 verbatim. The Bot contract is imported from
// `../../decision`. Exposed as `v32Bot`.
// ===========================================================================
 ...
 *
 *  v17 change (vs the v14 base): `./valuation.ts` LOWERS the voluntary-spend
 *  liquidity reserve (`FLOOR_RENT_FRACTION` 0.5→0.3, `FLOOR_CAP` 500→300) — the
 *  aggressive direction on the liquidity axis (v9 raised it and regressed). The bot
 *  keeps a thinner buffer and deploys more cash into buying/development sooner. The
 *  dispatcher and the whole trade engine are carried verbatim from v14. */
...
function preRoll(state: GameState, pid: string): BotDecision | null {
  ...
  // v32: `proposeBestTrade` returned null. If that's because a +EV distress
  // completion is blocked SOLELY by cash I could MORTGAGE to (not a general
  // shortfall), pre-raise the cash now via a mortgage-only `manage`; next turn the
  // completion clears through Offer B. Self-limited to one raise — once mortgaged,
  // cash covers the sweetener and the detector returns null. Own-turn only (a
  // manage arm is own-turn, like a build).
  if (state.turn.playerId === pid) {
    const raise = distressRaisePlan(state, pid);
    if (raise !== null) {
      return {
...
interface DistressRaise {
  /** position -> true: idle lots to mortgage to raise the completion's shortfall. */
  mortgage: Record<number, boolean>;
  reason: string;
}

/** v32 — the mortgage-only `manage` plan to fund a distress completion the bot
 *  can't pay for in cash but can reach by mortgaging idle lots. Returns null
 *  unless `distressCompletionNeedingCash` flags such a completion AND the shortfall
 *  is fundable by `planRaiseByMortgage` (least-essential lots first, monopolies
 *  protected). Build is empty — this raises cash only. */
function distressRaisePlan(state: GameState, pid: string): DistressRaise | null {
  const opp = distressCompletionNeedingCash(state, pid);
  if (opp === null) return null;
  const mortgage = planRaiseByMortgage(state, pid, opp.need);
  if (mortgage === null || Object.keys(mortgage).length === 0) return null;
  return {
    mortgage,
    reason: `Mortgaging idle land to raise cash for a cheap ${opp.color} completer off a cash-strapped owner.`,
  };
}
...
// set-completer / denial), capped by what I can actually pay. Bids are silent;
// dropping out gets one explanatory note.
// ---------------------------------------------------------------------------
...
function managing(state: GameState, pid: string): BotDecision | null {
  ...
  // v32: a distress-raise was armed if a build plan isn't what brought us here.
  // Commit the mortgage-only raise first (it funds next turn's completion); the
  // build plan (if any was the real arm) commits on the normal path. Both are
  // note-less — the arm explained the plan.
  const raise = distressRaisePlan(state, pid);
  if (raise !== null) {
    return {
      intent: { kind: "manage", playerId: pid, build: {}, mortgage: raise.mortgage },
    };
  }
  const plan = planBuild(state, pid);
