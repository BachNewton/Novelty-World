// ===========================================================================
// v32 SNAPSHOT — fork of v28 (the champion) (the phantom-denial-fixed base; see EVOLUTION.md). The
 ...
 *
 *  v32 change (vs the v28 base): the auction handler bids ABOVE its normal value
 *  cap when a distressed opponent is also bidding — starving a weak rival of
 *  new assets keeps them distressed (which feeds v28's discount-buying edge).
 *  Asymmetric auction denial, a channel v5's denial never covered. */
...
function auction(state: GameState, pid: string): BotDecision | null {
  ...
  // v32: bump the bid cap when a distressed opponent is still bidding — starving
  //  them of new assets keeps them weak and feeds the discount-buying edge.
  let vultureBoost = 0;
  for (const opp of activeOpponents(state, pid)) {
    if (a.active.includes(opp.id) && opp.id !== pid) {
      vultureBoost = Math.max(vultureBoost, Math.round(acquisitionValue(state, pid, a.position) * sellerDistress(state, opp.id) * 0.3));
    }
  }
  const cap = Math.min(acquisitionValue(state, pid, a.position) + vultureBoost, auctionBidCap(state, pid));
  if (next <= cap) {
