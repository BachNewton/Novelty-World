// ===========================================================================
// v13 SNAPSHOT — fork of the champion v5 (see EVOLUTION.md). The dispatcher is
// VERBATIM from v5 except the incoming-trade vote (`tradePending`): it now prices
// the rival-monopoly threat with the ANTI-KINGMAKER standings weight
// (`kingmakerThreatCost`) instead of v5's flat threat — the bot is extra loath to
// hand its STRONGEST opponent a monopoly, more willing to feed a harmless trailer.
// The change lives in `./trades.ts`; construction and the counterparty model keep
// the flat threat (exactly v5). Exposed as `v13Bot` via `./index.ts`.
// ===========================================================================
...
import { evaluateTrade, kingmakerThreatCost, proposeBestTrade } from "./trades";
 ...
 *
 *  v13 change (vs the v5 base): the bot's incoming-trade VOTE now prices the
 *  rival-monopoly threat by the recipient's STANDING (the anti-kingmaker weight in
 *  `./trades.ts`) — refusing to hand its strongest opponent a set more readily than
 *  it refuses a trailer. v5's trade-to-deny construction and the counterparty model
 *  are carried verbatim; only the bot's own acceptance vote changes. */
...
// position and don't hand a rival a stronger monopoly than I get. v13: the
// rival-monopoly threat is priced with the ANTI-KINGMAKER standings weight, so a
// deal that would feed the bot's strongest opponent is harder to clear than one
// feeding a harmless trailer. Only THIS vote is reweighted — construction and the
// counterparty model keep v5's flat threat (see `./trades.ts`).
// ---------------------------------------------------------------------------
function tradePending(state: GameState, pid: string): BotDecision | null {
  ...
  const verdict = evaluateTrade(state, pid, pending, kingmakerThreatCost);
