// ===========================================================================
// v14 SNAPSHOT — fork of the champion v5 (see EVOLUTION.md). The dispatcher is
// VERBATIM from v5; v14's only change is a correctness gate in `./trades.ts` that
// fixes the PHANTOM-DENIAL bug (Finding 1) — Offer C now requires the rival could
// REALISTICALLY acquire the completer before booking a denial premium, killing the
// bot→bot hot-potato of weak lots. The Bot/BotDecision CONTRACT is imported from
// the canonical `../../decision`, not snapshotted. Exposed as `v14Bot`.
// ===========================================================================
 ...
 *
 *  v14 change (vs the v5 base): the trade-to-deny construction (Offer C) now gates
 *  on whether the rival could REALISTICALLY acquire the completer it's being denied
 *  — fixing a phantom-denial bug where an already-blocked weak lot hot-potatoes
 *  between bots, each re-booking the denial premium. The dispatcher, `evaluateTrade`,
 *  completion search, and the denial pricing for sets that pass the gate are v5
 *  verbatim — the change is isolated to `./trades.ts`. */
