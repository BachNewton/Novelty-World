// ===========================================================================
// v22 SNAPSHOT — fork of the champion v17 (see EVOLUTION.md). The dispatcher and trade
// engine are VERBATIM; v22's only change is in `./valuation.ts` `desiredLevel` — a
// HOUSE-FAMINE DENIAL lever: it starts holding at 4-and-hold (rather than hoteling, which
// frees houses back to the bank) while the 32-house bank is merely DRAWING DOWN (≤12) and
// a rival could use houses, to win the race to starve rivals' development — sacrificing
// its own hotel rent for the denial. Carries v17's lower reserve verbatim. The Bot
// contract is imported from `../../decision`, not snapshotted. Exposed as `v22Bot`.
// ===========================================================================
 ...
 *
 *  v22 change (vs the v17 base): `./valuation.ts` `desiredLevel` adds a HOUSE-FAMINE
 *  DENIAL lever — it holds at 4-and-hold (not hotel, which frees houses) while the bank
 *  is drawing down (≤12) and a rival could use houses, to win the race to starve rivals'
 *  development, foregoing its own hotel rent. v17's lower reserve, planBuild, and the
 *  whole trade engine are carried verbatim. */
