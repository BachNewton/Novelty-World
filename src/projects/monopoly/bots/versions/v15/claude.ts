// ===========================================================================
// v15 SNAPSHOT — fork of v14 (the phantom-denial-fixed base; see EVOLUTION.md). The
// dispatcher is VERBATIM except the incoming-trade vote (`tradePending`): it now
// charges the NEAR-MONOPOLY OPTION VALUE (Finding 2) for foreclosing the bot's own
// one-short completion shot — so the bot holds its half of a mutual-blocker standoff
// unless the cash is a clear overpay. The change lives in `./trades.ts`; construction
// and the counterparty model keep v14/v5 valuation. Exposed as `v15Bot`.
// ===========================================================================
 ...
 *
 *  v15 change (vs the v14 base): the incoming-trade VOTE now charges a near-monopoly
 *  OPTION VALUE for foreclosing the bot's own one-short completion shot, so it won't
 *  sell its half of a mutual-blocker standoff for a song (Finding 2). v14's
 *  phantom-denial gate and construction are carried verbatim; only the bot's own
 *  acceptance vote changes (the option charge lives in `./trades.ts`). */
...
// position and don't hand a rival a stronger monopoly than I get. v15: the vote
// also charges the near-monopoly OPTION VALUE, so a deal that would foreclose my own
// one-short shot at a set must clear that cost too — I won't sell my half of a
// standoff cheaply. Only this vote is option-aware; construction + the counterparty
// model keep plain valuation (see `./trades.ts`).
// ---------------------------------------------------------------------------
function tradePending(state: GameState, pid: string): BotDecision | null {
  ...
  const verdict = evaluateTrade(state, pid, pending, true);
