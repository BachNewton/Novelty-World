...
// ===========================================================================
// v15 SNAPSHOT — fork of v14 (the phantom-denial-fixed base; see EVOLUTION.md).
// v14's Offer C reachability gate is carried VERBATIM; v15 adds ONE term — a
// NEAR-MONOPOLY OPTION VALUE on the bot's own incoming-trade vote (live-game
// Finding 2).
//
// THE GAP: `positionValue` credits only COMPLETED sets, so a lone lot in a color the
// bot is one-short of is valued at `assetBase` only. In a mutual-blocker standoff
// (bot and a rival each hold part of a color, mutually blocking), the bot will SELL
// its half for cash + the sweetenable rival-threat premium — foreclosing its OWN
// potential set for $0 on the books. A human exploits this by always being the BUYER:
// it refuses to sell as seller, then proposes the mirror as buyer, and the bot bites,
// handing the human every contested set for cash. Self-inconsistent: as PROPOSER the
// bot wants to complete that set; as SELLER it forecloses it cheaply.
//
// THE CHANGE (`optionForeclosureCost`): when a trade would drop the bot from
// one-short of a color (N-1 lots — a live completion shot) to further short, charge
// the bot's own vote `OPTION_FACTOR × monopolyBonus` for foreclosing that shot — on
// TOP of v5's rival-threat. So the bot holds a contested near-monopoly lot unless the
// cash is a clear overpay. NOTE the live twist (Finding 2): a human won BOTH sets via
// this exploit and still went bankrupt, so monetizing a contested completer can be
// correct — this is gauntlet-arbitrated, expected to be neutral-or-better and, per the
// "best of the best" charter, less obviously exploitable by a human either way.
//
// SCOPE (mirrors v13/v14 discipline): the option charge applies ONLY to the bot's own
// incoming-trade vote (`evaluateTrade` with `chargeOptionValue=true`). Construction
// and the counterparty model keep plain v14/v5 valuation, so the bot doesn't
// mis-model the non-option-aware field. v14's phantom-denial gate is untouched.
// ---------------------------------------------------------------------------
...
/** v15 NEAR-MONOPOLY OPTION VALUE: when a trade forecloses the bot's own one-short
 *  completion shot on a color, charge its vote this fraction of that set's bonus.
 *  Mirrors `DENY_FACTOR` (a fractional credit for a positional option that isn't a
 *  completed set): a one-short position isn't a monopoly, but it's a real shot at
 *  one, worth holding unless the cash clearly overpays. Deliberately < 1 so a genuine
 *  overpay for a contested completer still clears (the Finding-2 twist: monetizing a
 *  set you can't develop can be correct). */
const OPTION_FACTOR = 0.6;
...
/** v15 — what `pid` forecloses of its OWN sets by this trade: for each color it was
 *  one-short of BEFORE (owned N-1 — a live completion shot) but holds FEWER of AFTER,
 *  `OPTION_FACTOR × monopolyBonus`. `positionValue` credits only completed sets, so
 *  without this a near-monopoly lot reads as mere `assetBase` and the bot sells its
 *  half of a mutual-blocker standoff for a song. Charged only on the bot's own vote. */
function optionForeclosureCost(before: GameState, after: GameState, pid: string): number {
  let cost = 0;
  for (const color of COLORS_BY_WEIGHT) {
    const n = groupPositions(color).length;
    if (ownedInColor(before, pid, color) === n - 1 && ownedInColor(after, pid, color) < n - 1) {
      cost += monopolyBonus(color) * OPTION_FACTOR;
    }
  }
...
export function evaluateTrade(
  ...
  const optionCost = chargeOptionValue ? optionForeclosureCost(state, after, pid) : 0;
  const delta = rawDelta - threatCost - optionCost;
  ...
  if (delta <= ACCEPT_MIN) {
    return {
      ...
      reason:
        optionCost > 0
          ? "it would foreclose my own shot at the set"
          : threatCost > 0
