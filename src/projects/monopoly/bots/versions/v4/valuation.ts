// ===========================================================================
// v4 SNAPSHOT — a self-contained copy of the bot's policy (see EVOLUTION.md).
// Forked from v3 (N-way trades + v2 rival-threat pricing, all in `./trades.ts`,
// carried here verbatim). v4's ONE behavioral change lives in this file, in
// `planBuild`: TEMPO via mortgage-funded development.
//
// v4 HYPOTHESIS: being first to high rent on a PRIZE monopoly wins games. v3 made
// set-completion symmetric (both sides can now assemble N-way), which proved
// win-NEUTRAL because the residual deadlock cost draws, not losses — a wash. To
// win share you need ASYMMETRY. The most direct one is development TEMPO: whoever
// turns a fresh monopoly into crushing rent FIRST starts bankrupting rivals first.
//
// Today `planBuild` funds development ONLY from cash above the liquidity floor, so
// a bot that just spent its cash COMPLETING a set (via a trade sweetener) then
// develops that prize set slowly — capped at a house or two — while idle,
// non-earning back-burner lots (a lone property in an unfinished set, a spare
// railroad/utility) sit mortgageable. v4 lets the bot MORTGAGE its least-essential,
// building-free, NON-monopoly lots to fund developing a prize set a level sooner —
// converting idle equity into rent pressure ahead of rivals. Crucially it does NOT
// lower the reserve: the funded build still ends at or above the SAME liquidity
// floor (`cash + netCash >= floor`); it just stops WAITING to accumulate cash when
// idle capital is sitting there to redeploy. Monopolies are never cannibalized
// (only non-monopoly lots are eligible), and the leverage is gated to real sets
// (`TEMPO_PRIZE_BONUS`) and real builds (`TEMPO_MIN_LEVEL`) so it never burns
// railroad income to put a token house on a weak set.
//
// Shared, non-strategic infrastructure (engine, board geometry) is still imported
// from the canonical modules — only the *policy* is snapshotted.
// ===========================================================================
...
const HOTEL_CUSHION = 600;

/** v4 TEMPO lever — mortgage idle back-burner capital to develop sooner. Only a
 *  set whose monopoly bonus clears this is worth leveraging for; it excludes the
 *  trivial brown set (bonus ~84), so the bot never burns a railroad's rent to put
 *  a token house on the weakest color. Every real income set (light-blue ~320 and
 *  up) qualifies. */
const TEMPO_PRIZE_BONUS = 150;

/** v4 TEMPO lever — only mortgage-fund a build that reaches at least this level
 *  (the rent-per-dollar sweet spot). Leveraging idle capital to creep a prize set
 *  to one or two houses isn't a tempo edge worth the interest and lost rent; the
 *  cash-funded path already handles those low builds. */
const TEMPO_MIN_LEVEL = 3;
...
export interface BuildPlan {
  ...
  /** position -> flag, the mortgage side of this commit (engine applies it
   *  raise-first against the post-flip state — `applyManageCommit`). Two kinds of
   *  entry, both empty for a plain cash-funded build:
   *   - `false`: a mortgaged monopoly member this plan LIFTS before developing it
   *     (a dead, mortgaged set earns nothing until unmortgaged — reclaiming idle
   *     capital, part of the same commit as the build).
   *   - `true` (v4 TEMPO): a least-essential, building-free, NON-monopoly back-burner
   *     lot this plan MORTGAGES to fund developing a prize set a level sooner. Idle
   *     equity → rent pressure, without dropping below the liquidity floor. */
 ...
 *  past raise; a thin bot leaves the set mortgaged.
 *
 *  v4 TEMPO: when cash above the floor can't reach a prize set's desired level,
 *  the planner mortgages idle, non-monopoly back-burner lots to fund the build a
 *  level sooner (`fundShortfall`) — the funded commit still clears the floor, so
 *  this redeploys idle capital rather than dipping the reserve. Tried per level
 *  from the desired one down, so it always prefers the highest reachable level and
 *  mortgages the fewest, least-essential lots needed.
 *
 *  Returns null when nothing is worth (or can be afforded) now — which makes the
 *  pre-roll arm decision and the commit identical and keeps the bot from spinning. */
export function planBuild(state: GameState, pid: string): BuildPlan | null {
  ...
  for (const color of COLORS_BY_WEIGHT) {
    ...
    // Try the highest reachable even level from `want` down to `stop`, validating
    // the WHOLE accumulated commit (build + unmortgages [+ tempo funding]) each step.
    for (let level = want; level >= stop; level--) {
      ...
      // 1. Cash-funded — the highest level affordable above the floor from cash.
      if (affordableBuild(state, pid, candidateBuild, candidateMortgage, floor)) {
        if (level > 0) for (const pos of positions) build[pos] = level;
        for (const pos of mortgagedMembers) mortgage[pos] = false;
        reasons.push(redeployReason(color, level, locked, why, level === want, false));
        break;
      }

      // 2. v4 TEMPO — for a prize set and a real build, fund the shortfall by
      //    mortgaging idle, non-monopoly back-burner lots (still ending above the
      //    floor). Develops the prize set a level sooner than waiting for cash.
      if (monopolyBonus(color) >= TEMPO_PRIZE_BONUS && level >= TEMPO_MIN_LEVEL) {
        const funded = fundShortfall(state, pid, candidateBuild, candidateMortgage, floor);
        if (funded !== null) {
          if (level > 0) for (const pos of positions) build[pos] = level;
          for (const [posStr, flag] of Object.entries(funded)) mortgage[Number(posStr)] = flag;
          reasons.push(redeployReason(color, level, locked, why, level === want, true));
          break;
        }
      }
    }
 ...
 *  clean build-up from reclaiming a mortgaged set (with or without houses), and
 *  flagging a v4 tempo build funded by mortgaging idle lots. */
function redeployReason(
  ...
  const fund = funded ? ", funded by mortgaging idle lots" : "";
  if (locked) {
    return level === 0
      ? `unmortgaging ${name} to restore its monopoly rent${fund}`
      : `unmortgaging ${name} and building to ${levelPhrase(level)}${fund}`;
  }
  const rationale = atTarget ? why : funded ? "to get ahead on rent" : "as far as cash allows";
  return `${name} to ${levelPhrase(level)} (${rationale}${fund})`;
}

// ---------------------------------------------------------------------------
// v4 TEMPO funding. Mortgage idle, non-monopoly back-burner capital to develop a
// prize set sooner — never cannibalizing a monopoly, never dipping the reserve.
// ---------------------------------------------------------------------------

/** Does a staged build+mortgage commit pass — legal AND leaving `pid` at or above
 *  its liquidity floor? The single affordability gate, shared by the cash-funded
 *  and tempo-funded paths so they judge a commit identically. */
function affordableBuild(
  state: GameState,
  pid: string,
  ...
  const summary = manageSummary(state, pid, { build, mortgage });
  return summary.ok && player.cash + summary.netCash >= floor;
}

/** `pid`'s idle back-burner lots, least-essential first: owned, building-free,
 *  un-mortgaged, NOT part of a monopoly I hold (essentialness < 4 — a monopoly lot
 *  is the engine, never mortgaged to fund another set), and not already staged
 *  (`exclude`). Ordered by essentialness then mortgage value so a utility goes
 *  before a lone property before a railroad, cheapest within a tier — the same
 *  value-preserving order the forced-liquidation path uses. */
function eligibleFundingLots(
  state: GameState,
  pid: string,
  exclude: ReadonlySet<number>,
): number[] {
  const lots: { pos: number; ess: number; value: number }[] = [];
  for (const [posStr, owner] of Object.entries(state.ownership)) {
    if (owner !== pid) continue;
    const pos = Number(posStr);
    if (exclude.has(pos) || state.mortgaged[pos]) continue;
    if (builtLotsInGroup(pos, (p) => developmentLevel(state, p)).length > 0) continue;
    if (essentialness(state, pid, pos) >= 4) continue;
    const value = mortgageValueAt(pos);
    if (value === null) continue;
    lots.push({ pos, ess: essentialness(state, pid, pos), value });
  }
  lots.sort((a, b) => a.ess - b.ess || a.value - b.value);
  return lots.map((l) => l.pos);
}

/** Augment `baseMortgage` (the set's redeploy unmortgages already in it) with the
 *  fewest back-burner mortgages needed to make `build` affordable above the floor,
 *  or null if even mortgaging every eligible lot falls short. Adds lots one at a
 *  time, least-essential first, re-checking the WHOLE commit each step so it stops
 *  at the minimal funding — the tempo bet is bounded, not all-in. */
function fundShortfall(
  state: GameState,
  pid: string,
  build: Readonly<Record<number, number>>,
  baseMortgage: Readonly<Record<number, boolean>>,
  floor: number,
): Record<number, boolean> | null {
  const mortgage: Record<number, boolean> = { ...baseMortgage };
  const exclude = new Set(Object.keys(mortgage).map(Number));
  for (const pos of eligibleFundingLots(state, pid, exclude)) {
    if (affordableBuild(state, pid, build, mortgage, floor)) break;
    mortgage[pos] = true;
  }
  return affordableBuild(state, pid, build, mortgage, floor) ? mortgage : null;
}
