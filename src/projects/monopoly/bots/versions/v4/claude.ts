// ===========================================================================
// v4 SNAPSHOT — self-contained fork of `bots/claude.ts` (see EVOLUTION.md). The
// dispatcher is verbatim; v4's behavioral change is in `./valuation.ts`
// (`planBuild`): TEMPO via mortgage-funded development — develop a prize set a
// level sooner by mortgaging idle, non-monopoly back-burner lots. The trade
// engine (`./trades.ts`: N-way construction + v2 rival-threat pricing) is carried
// over from v3 verbatim. The Bot/BotDecision CONTRACT is stable shared
// infrastructure, so it is imported from the canonical `../../decision`, not
// snapshotted. Exposed as `v4Bot` via `./index.ts`.
// ===========================================================================
 ...
 *
 *  v4 change (vs the v3 base): development is now TEMPO-driven. When cash above
 *  the liquidity floor can't reach a prize set's desired level, `planBuild`
 *  mortgages idle, non-monopoly back-burner lots to fund the build a level sooner
 *  (`./valuation.ts`), so the bot turns a fresh monopoly into rent pressure ahead
 *  of rivals instead of waiting to accumulate cash. Inherited from v3/v2: N-way
 *  trade construction and the priced (not vetoed) rival-monopoly threat
 *  (`./trades.ts`). The dispatcher itself is verbatim.
 *
 *  Known limits (deliberate): a TRADE sweetener is still cash-funded only — the
 *  mortgage-to-fund path covers builds, not trade cash. */
