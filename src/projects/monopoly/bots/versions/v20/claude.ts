// ===========================================================================
// v20 SNAPSHOT — fork of the champion v17 (see EVOLUTION.md). Carries v17's lower
// liquidity reserve and the whole trade engine verbatim; v20's ONE change is HERE in
// the dispatcher: it lowers `DIP_WORTH_MULT` (1.4→1.15), loosening the BUY-AGGRESSION
// gate so the bot dips below its (already thin) reserve to acquire land of clear value
// more readily. This is the ACQUISITION analog of v17's winning "hold less idle cash"
// — and FREE (it spends cash on hand, no mortgage interest, unlike the v4/v8 tempo that
// washed). The Bot contract is imported from `../../decision`. Exposed as `v20Bot`.
// ===========================================================================
 ...
 *
 *  v20 change (vs the v17 base): lower `DIP_WORTH_MULT` (1.4→1.15) below — the bot
 *  dips into its rent reserve to BUY land of clear value (set progress, railroad
 *  synergy, denial) more readily, the acquisition analog of v17's thinner reserve and
 *  free (cash on hand, no interest). v17's lower reserve, valuation, and the whole
 *  trade engine are carried verbatim. */
...
/** How willing to dip below the liquidity reserve for an affordable buy: v20
 *  LOOSENS this (1.4→1.15) so the bot dips for any lot of clear value (set
 *  progress, railroad synergy, denial), not only ones worth well over face — the
 *  acquisition analog of v17's thinner reserve, funded by cash on hand (free, no
 *  mortgage interest). The reserve still governs the threshold; this only lowers
 *  how much above face a buy must be worth to justify spending into it. */
const DIP_WORTH_MULT = 1.15;
