// ===========================================================================
// v21 SNAPSHOT — fork of the CHAMPION v17 (see EVOLUTION.md). COUPLES the two
// leaning-positive liquid-deployment near-misses (both ~+13 Elo sub-threshold): v18's
// thinner reserve (in `./valuation.ts`: 0.3→0.15, cap 300→200, base 120→80) AND v20's
// looser buy-dip gate (HERE: `DIP_WORTH_MULT` 1.4→1.15). One coherent hypothesis —
// v17 leaves a small liquid-deployment edge on BOTH gates, and loosening them together
// crosses E=20. The trade engine is verbatim v17/v14. The Bot contract is imported from
// `../../decision`, not snapshotted. Exposed as `v21Bot`.
// ===========================================================================
 ...
 *
 *  v21 change (vs the v17 champion): COUPLES the two leaning-positive liquid-deployment
 *  near-misses — `./valuation.ts`'s thinner reserve (v18: 0.3→0.15, cap 300→200, base
 *  120→80) AND the looser buy-dip gate below (v20: `DIP_WORTH_MULT` 1.4→1.15). One
 *  coherent hypothesis: v17 leaves a small liquid-deployment edge on both gates that
 *  crosses E=20 only when loosened together. The trade engine is verbatim v17/v14. */
...
/** How willing to dip below the liquidity reserve for an affordable buy: v21 carries
 *  v20's LOOSER gate (1.4→1.15) so the bot dips for any lot of clear value (set
 *  progress, railroad synergy, denial), not only ones worth well over face — the
 *  acquisition half of v21's coupled liquid-deployment loosening, funded by cash on
 *  hand (free, no mortgage interest). */
