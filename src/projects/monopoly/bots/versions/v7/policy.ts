// ===========================================================================
// v7 SNAPSHOT — self-contained fork of `bots/policy.ts` (see EVOLUTION.md).
// Branched from v5. The dispatcher is VERBATIM from v3/v5; v7's behavioral change
// is entirely in `./trades.ts` (EARLY / two-short denial construction). The
// Bot/BotDecision CONTRACT is stable shared infrastructure, so it is imported from
// the canonical `../../decision`, not snapshotted. Exposed as `v7Bot` via `./index.ts`.
// ===========================================================================
 ...
 *
 *  v7 change (vs the v5 base): the trade-to-deny construction (v5) now also blocks
 *  a rival who is TWO lots short of a strong set — taking one of the two missing
 *  lots from a third-party holdout makes the set impossible, caught early while the
 *  lots are still distributed and cheap (and before the rival, going one-short,
 *  grabs the completer itself and puts it out of reach). The credit is discounted
 *  for the extra distance. Everything else (the dispatcher, `evaluateTrade`,
 *  N-way completion, v5's one-short denial gate) is verbatim — the change is
 *  isolated to `./trades.ts`. */
