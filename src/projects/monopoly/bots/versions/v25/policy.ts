// ===========================================================================
// v25 SNAPSHOT — fork of the champion v17 (see EVOLUTION.md). This dispatcher and
// `./valuation.ts` are carried VERBATIM from v17 (valuation only gains two new
// exported helpers, `ownedOfKind` + `kindCompletionBonus`); v25's change lives in
// `./trades.ts` — ONE defensive thing: `rivalThreatCost` now prices handing a rival
// their 4th railroad / 2nd utility, so the bot won't SELL a rail-set completer for
// face value. (Proactive rail denial was dropped — it hot-potatoes; see trades.ts.)
// The Bot contract is imported from `../../decision`. Exposed as `v25Bot`.
// ===========================================================================
 ...
 *
 *  v25 change (vs the v17 base): `./trades.ts` `rivalThreatCost` now prices handing
 *  a rival their 4th railroad / 2nd utility, so the bot won't sell a rail-set
 *  completer for face value (v17 priced only color monopolies). Proactive rail
 *  denial was dropped (it hot-potatoes). The dispatcher and `./valuation.ts` (bar
 *  two new exported helpers) are carried verbatim from v17. */
