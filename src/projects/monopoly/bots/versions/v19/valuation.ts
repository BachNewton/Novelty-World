// ===========================================================================
// v19 SNAPSHOT — fork of the champion v17 (see EVOLUTION.md). Carries v17's lower
// liquidity reserve verbatim and adds ONE change, here: ENDGAME ELIMINATION PRESSURE.
//
// THE LEAD: the only outcome that counts is bankrupting opponents (EVOLUTION.md
// "Winning is bankruptcy"), yet the bot never plays to FINISH a specific weakened
// rival — it develops on a flush/ROI heuristic blind to whether a kill is on offer.
// v19 keys development off rival DISTRESS: when an active rival is on the ropes —
// their raisable cash (`cash + mortgageableTotal`) can't cover my deadliest developed
// rent, so one landing on my board ends them — the bot enters ELIMINATION MODE and
// pushes its monopolies to MAX rent (hotels, or 4-and-hold under a house shortage)
// regardless of "flush", deploying its cushion into finishing the kill. The bet:
// against a near-broke rival, cranking rent NOW multiplies the lethal squares and
// closes the game before variance (a GO lap, a lucky trade) lets them recover —
// converting a likely win into a decisive bankruptcy and cutting the luck that can
// swing an already-won game. Proactive, negative-sum, the proven winning shape, and
// untried. The change is isolated to `desiredLevel`; everything else (the lower
// reserve, planBuild, the whole trade engine) is v17 verbatim.
// ===========================================================================
...
/** The worst single rent currently sitting on MY board — the deadliest square a
 *  rival could land on and owe me. `rentEstimateAt` already resolves monopoly
 *  double-rent, house/hotel rent, and railroad scaling (and returns 0 for a
 *  mortgaged lot), so this is the largest hit I can presently deal. */
function myDeadliestRent(state: GameState, pid: string): number {
  let worst = 0;
  for (const [posStr, owner] of Object.entries(state.ownership)) {
    if (owner !== pid) continue;
    worst = Math.max(worst, rentEstimateAt(state, Number(posStr)));
...
/** v19 — is an active rival ON THE ROPES (a kill genuinely on offer)? True when
 *  some opponent's raisable cash — everything they could put on the table by
 *  mortgaging (`cash + mortgageableTotal`, before tearing down houses, which a
 *  near-broke rival rarely has) — can't cover my deadliest developed rent: one
 *  landing on my board ends them. When that's true, escalating my development to
 *  max rent multiplies the lethal squares and finishes the kill before variance
 *  lets them recover. Gated on my actually HAVING a lethal square
 *  (`myDeadliestRent > 0`); early, with cheap rents and flush rivals, it's a
 *  no-op and v19 plays exactly as v17. */
function rivalOnTheRopes(state: GameState, pid: string): boolean {
  const kill = myDeadliestRent(state, pid);
  if (kill === 0) return false;
  return activeOpponents(state, pid).some(
    (opp) => opp.cash + mortgageableTotal(state, opp.id) < kill,
  );
}
 ...
 *  - (v19) a rival ON THE ROPES → hotel for MAX rent even when not flush, to
 *    finish the kill — `planBuild` funds it down to the reserve floor, deploying
 *    my cushion into the rent that bankrupts them;
 ...
 *  The scarcity branch wins over elimination on purpose: hotels under a house
 *  shortage hand four houses back to the bank for the rival to buy, so 4-and-hold
 *  is already the max rent I should take there. */
function desiredLevel(
  ...
  if (rivalOnTheRopes(state, pid)) {
    return { level: 5, why: "going to hotels to finish off a rival on the ropes" };
  }
  if (flush && !scarce) {
