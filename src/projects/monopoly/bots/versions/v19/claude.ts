// ===========================================================================
// v19 SNAPSHOT — fork of the champion v17 (see EVOLUTION.md). The dispatcher and trade
// engine are VERBATIM; v19's only change is in `./valuation.ts` — ENDGAME ELIMINATION
// PRESSURE: when a rival is on the ropes (one landing on my board would end them), the
// build judgment (`desiredLevel`) pushes my monopolies to MAX rent even when not flush,
// deploying my cushion into finishing the kill. Carries v17's lower reserve verbatim.
// The Bot contract is imported from `../../decision`, not snapshotted. Exposed as `v19Bot`.
// ===========================================================================
 ...
 *
 *  v19 change (vs the v17 base): `./valuation.ts` adds ENDGAME ELIMINATION PRESSURE —
 *  when an active rival is on the ropes (their raisable cash can't cover my deadliest
 *  developed rent, so one landing ends them), the build judgment pushes my monopolies
 *  to MAX rent (hotels) even when not flush, deploying my cushion into finishing the
 *  kill. v17's lower reserve, planBuild, and the whole trade engine are verbatim. */
