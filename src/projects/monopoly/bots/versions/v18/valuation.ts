// ===========================================================================
// v18 SNAPSHOT — fork of the CHAMPION v17 (see EVOLUTION.md). v17 won by lowering
// the liquidity reserve (the aggressive direction on the liquidity axis); v18 PUSHES
// that same winning lever further — `FLOOR_RENT_FRACTION` 0.3→0.15, `FLOOR_CAP`
// 300→200, `BASE_FLOOR` 120→80 — to find where the aggression stops paying. The
// trade engine (v14's phantom-denial gate) and everything else are carried verbatim;
// only the three floor constants change here. Shared infrastructure is imported from
// the canonical modules — only the policy is snapshotted.
// ===========================================================================
 ...
 *
 *  v17 LOWERED this reserve (v5's 0.5/cap-500 → 0.3/cap-300) and WON — the aggressive
 *  direction on the liquidity axis paid off (v9 had RAISED it and regressed). v18
 *  PUSHES the same winning lever further (0.3→0.15, cap 300→200, base 120→80): if
 *  v5's reserve was too cautious, is v17's still too cautious? This finds where the
 *  aggression stops paying — either an even thinner buffer keeps winning (deploy yet
 *  more cash sooner) or it overshoots (too thin to absorb a hit without a damaging
 *  fire-sale), pinpointing the optimum on the reserve axis. */
