...
// from FIRST PRINCIPLES — the expected rent a completed-and-developed set earns
// per opponent turn, relative to the capital sunk to reach that state (ROI):
//
//   monopolyBonus(color) = BONUS_SCALE × Σ_lot (landingProb[lot] × rent3[lot])
//                                        ──────────────────────────────────────
//                                          Σ_lot (price[lot] + 3 × houseCost)
//
//   i.e. (expected 3-house rent per opponent roll) / (capital to acquire + build
//   the set to 3 houses), scaled into position-value dollars by BONUS_SCALE.
//
// WHY ROI, not raw expected rent: a pure landing-prob × developed-rent sum
// (without the capital denominator) ranks the EXPENSIVE high-rent sets
// (green/yellow/dark-blue) above orange/red, because absolute 3-house rent
// dwarfs the modest traffic differences between sets — which contradicts both
// pro practice and the sanity gate (orange/red must stay near the top). Dividing
// by the capital to reach the state is the literature's actual ROI ranking axis,
// and it puts ORANGE at #1 with red in the upper tier — grounded in measured
// landing frequencies, not intuition. See the landingProb source note below.
//
...
// gate). monopolyBonus keeps the same signature and ~magnitude (orange ≈ 560),
// so DENY_FACTOR, ACCEPT_MARGIN and the liquidity floors operate on the same
// scale; only the RELATIVE ranking of sets shifts to the measured ROI.
// ===========================================================================
...
/** Strategic weight per color set — a relative "how precious is this set" ranking.
 *  v38: this NO LONGER feeds `monopolyBonus` (that is now derived from expected-rent
 *  ROI; see `MONOPOLY_BONUS`). It survives only as the LIQUIDATION-PRECEDENCE weight
 *  in `raiseCashStep` (sell the lowest-weight developed set first) — a deliberately
 *  separate axis from a set's earning power, kept verbatim so the forced-sale order
 *  is unchanged by the valuation rework. Railroads/utilities are scored elsewhere. */
 ...
 *  that set's monopoly bonus. claude-v36: 0.3 → 0.15 — denial was over-weighted
...
/** Steady-state landing probability for each color-set square, in PERCENT per
 *  opponent roll. These are the classic long-run Monopoly Markov-chain figures
 *  (Truman Collins' published board analysis — accounting for the Jail hub,
 *  Go-to-Jail, the "advance" Chance/CC cards, and the three-doubles rule). They
 *  are an APPROXIMATION (a fixed steady-state, not conditioned on live game
 *  state), which is all the valuation needs: the bonus is a static per-set
 *  ranking, not a live rent forecast. Only the developable color squares appear;
 *  GO/Jail/railroads/utilities are scored elsewhere. */
...
/** The 3-house rent of a developable square, read straight from the board data
 *  (`rent.houses[2]` — the third house). 3 houses is the bot's development floor
 *  (`SPREAD_FLOOR` in planBuild) and the steepest rent-per-dollar tier, so it is
 *  the natural reference level for a set's earning power. Zero for a non-property. */
function rent3House(pos: number): number {
  const s = SPACES[pos];
  return s.kind === "property" ? s.rent.houses[2] : 0;
}

/** BONUS_SCALE turns a dimensionless ROI (expected rent / capital) into
 *  position-value dollars. Pinned so ORANGE — the highest-ROI set — lands at 560,
 *  matching claude-v36's orange bonus, so DENY_FACTOR / ACCEPT_MARGIN / the
 *  liquidity floors all operate on the SAME magnitude as before; only the
 *  RELATIVE ranking of the sets changes to the measured ROI. */
const BONUS_SCALE = 16489;

/** The premium a COMPLETED monopoly adds to a position, above the raw asset base
 *  of its lots — now DERIVED from first principles (v38) rather than hand-tuned:
 *  the expected rent the set earns per opponent roll at 3 houses, divided by the
 *  capital sunk to acquire and develop it to 3 houses (an ROI), scaled into
 *  dollars by BONUS_SCALE. Grounded in measured landing frequencies
 *  (`LANDING_PROB`) and the board's real rents/prices — orange tops the ROI, the
 *  cheap high-traffic sets stay well-credited, the costly low-ROI sets (green)
 *  fall, and brown is weakest. Memoized — the inputs are all static. */
const MONOPOLY_BONUS: Readonly<Record<PropertyColor, number>> = (() => {
  ...
  for (const color of COLORS_BY_WEIGHT) {
    ...
    bonuses[color] = capital > 0 ? Math.round((expectedRent / capital) * BONUS_SCALE) : 0;
  }
  return bonuses;
})();

/** The premium a COMPLETED monopoly adds to a position, above the raw asset base
 *  of its lots. A bare orange monopoly is worth ~$560 more than its three deeds;
 *  brown ~$230. See `MONOPOLY_BONUS` for the expected-rent-ROI derivation. */
function monopolyBonus(color: PropertyColor): number {
  return MONOPOLY_BONUS[color];
}
