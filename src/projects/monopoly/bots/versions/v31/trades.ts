...
// v31 adds DISTRESS-WEIGHTED DENIAL in valuation.ts. The trade engine is v28
// verbatim. — fork of v17 (the champion; see EVOLUTION.md).
  ...
  for (const opp of activeOpponents(state, pid)) {
    for (const color of COLORS_BY_WEIGHT) {
      ...
      candidates.push({
        terms: buy,
        delta: 0,
        denyBonus: Math.round(monopolyBonus(color) * DENY_FACTOR * (1 + sellerDistress(state, opp.id))),
