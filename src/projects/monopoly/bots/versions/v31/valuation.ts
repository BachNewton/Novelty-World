// ===========================================================================
// v31 SNAPSHOT — fork of v28 (the champion; see EVOLUTION.md).
// v31 adds DISTRESS-WEIGHTED DENIAL: denying a distressed opponent's monopoly
// is worth more (they can't recover easily). Scales the DENY_FACTOR by opponent
// distress in acquisitionValue. — fork of v17 (the champion; see EVOLUTION.md).
  ...
  if (color !== null && !(pos in state.ownership)) {
    ...
    for (const opp of activeOpponents(state, pid)) {
      ...
      if (ownedInColor(state, opp.id, color) === total - 1) {
        const oppDistress = sellerDistress(state, opp.id);
        const denyWeight = DENY_FACTOR * (1 + oppDistress);
        deny = Math.max(deny, Math.round(monopolyBonus(color) * denyWeight));
      }
    }
  }
  return mine + deny;
