// ===========================================================================
// jane-v4 SNAPSHOT — fork of jane-v2 (see EVOLUTION.md).
// jane-v4 adds ENDGAME HOTEL CONCENTRATION: when in late-game state
// (all monopolies at 3+ houses or 100+ turns), dump ALL remaining budget into
// hotels on the single best set — a kill-shot weapon that spread development
// cannot match. jane-v2 spreads evenly, creating no knockout threat. — fork of jane-v1 (see EVOLUTION.md).
...
export function planBuild(state: GameState, pid: string): BuildPlan | null {
  ...
  // PASS 3 (ENDGAME): Concentrate ALL remaining budget on the single best set.
  // In late-game, both boards are developed and rents cancel out. A hotel on
  // the best set is a one-hit bankruptcy threat — the only way to actually
  // end long games. This is asymmetric: spread development (jane-v2) creates
  // no single knockout threat.
  const allAtThree = myMonopolies.length > 0 && myMonopolies.every(
    (m) => Math.max(...m.positions.map((p) => build[p] ?? developmentLevel(state, p))) >= 3
  );
  const lateGame = state.turns.length > 100;
  if ((allAtThree || lateGame) && myMonopolies.length > 0) {
    // Pick the best (highest weight) set that's not locked
    const target = myMonopolies.find((m) => !m.locked);
    if (target) {
      const currentLevel = Math.max(...target.positions.map((p) => build[p] ?? developmentLevel(state, p)));
      // Try to push it to hotels (level 5) — the kill shot
      for (let level = 5; level > currentLevel; level--) {
        if (tryLevel(target.color, target.positions, target.mortgagedMembers, level, target.locked)) {
          reasons.push(redeployReason(target.color, level, false, "endgame kill shot", true));
          break;
        }
      }
    }
