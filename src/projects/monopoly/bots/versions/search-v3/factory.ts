...
// ===========================================================================
// search-v2 ADDITION — `makeParamValue`: the SAME `positionValue` the factory's
// dispatcher uses internally, exposed standalone so the rollout-search LEAF can
// score truncated games by the TUNED champion's yardstick (not the untuned v38
// `positionValue` search-v1 used). This MIRRORS the monopolyBonus + positionValue
// computation inside `makeParamBot` verbatim; if you ever change one, change both
// (this snapshot is frozen, so they can't drift in practice). Kept separate
// rather than refactored out of the closure so `makeParamBot` stays byte-for-byte
// the claude-v45 factory — its fidelity is what makes the A/B a clean test.
// ===========================================================================
export function makeParamValue(p: ParamVector): (state: GameState, pid: string) => number {
  const railSynergyValues: readonly number[] = [0, 0, p.railSynergy2, p.railSynergy3, p.railSynergy4];
  const railSynergy = railSynergyValues.map((v) => Math.round(v * p.railSynergyScale));
  const monoMultByColor: Readonly<Record<PropertyColor, number>> = {
    orange: p.monoMultOrange,
  ...
  return function positionValue(state: GameState, pid: string): number {
    const player = state.players.find((q) => q.id === pid);
    if (!player) return 0;
    let value = player.cash;
    let rails = 0;
    ...
    for (const color of COLORS_BY_WEIGHT) {
      if (hasMonopoly(state, color, pid)) value += monopolyBonusByColor[color];
    }
    value += railSynergy[Math.min(rails, 4)];
    if (utils === 2) value += p.utilPairBonus;
    return value;
