...
// SURVIVAL_CAP, deliberately BELOW the full worst-case so the bot never goes
// fully passive — so it outlasts the variance and survives the hotel hit that
// busts a rival without fire-selling its own monopolies. It is a DEFENSIVE
// hardening (a liquidity guard), NOT a discount on what a monopoly is worth —
// `positionValue`/`acquisitionValue` are untouched (see bots/CLAUDE.md
// "Considered and rejected", which rejects the discount but flags a graduated
// liquidity guard as a reasonable hardening). Shared, non-strategic
// infrastructure (engine, board geometry) is still imported from the canonical
// modules — only the *policy* is snapshotted.
// ===========================================================================
...
/** v9 SURVIVAL guard. v5's floor (above) caps at FLOOR_CAP = $500 — moderate by
 *  design, but on a DEVELOPED board (a rival hotel can charge $1000–2000) that
 *  reserve is far short of a single hit, so the bot voluntarily spends down,
 *  takes the hit, and fire-sells its own monopolies through must-raise-cash to
 *  cover it. The variance that busts a rival then busts us too. The guard: when a
 *  rival board is DEVELOPED (houses/hotel), reserve a graduated share
 *  (`SURVIVAL_RENT_FRACTION`) of the worst DEVELOPED rent — enough to absorb the
 *  hit and keep fighting — but bounded by `SURVIVAL_CAP`, deliberately set BELOW
 *  the full worst-case hotel so the bot never reserves so much it stops developing
 *  (the passivity CLAUDE.md warns against). Below a developed board the term is
 *  zero, so behavior is EXACTLY v5 on an undeveloped board. */
const SURVIVAL_RENT_FRACTION = 0.8;
const SURVIVAL_CAP = 900;
...
export function liquidityFloor(state: GameState, pid: string): number {
  let worst = 0;
  let worstDeveloped = 0;
  for (const [posStr, owner] of Object.entries(state.ownership)) {
    if (owner === pid) continue;
    const pos = Number(posStr);
    const rent = rentEstimateAt(state, pos);
    worst = Math.max(worst, rent);
    if (developmentLevel(state, pos) > 0) worstDeveloped = Math.max(worstDeveloped, rent);
  }
  // v5's moderate reserve — half the worst rent on the board, capped low.
  const base = Math.min(FLOOR_CAP, Math.max(BASE_FLOOR, Math.round(worst * FLOOR_RENT_FRACTION)));
  // v9 SURVIVAL guard — a larger, graduated reserve when a DEVELOPED board
  // threatens, bounded below the full hit so the bot keeps developing. Zero (so
  // floor == v5's `base`) when no developed rival rent is out there.
  const survival =
    worstDeveloped > 0
      ? Math.min(SURVIVAL_CAP, Math.round(worstDeveloped * SURVIVAL_RENT_FRACTION))
      : 0;
  return Math.max(base, survival);
}
