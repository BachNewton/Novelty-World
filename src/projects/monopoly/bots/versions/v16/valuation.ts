// ===========================================================================
// v16 SNAPSHOT — fork of v14 (the phantom-denial-fixed base; see EVOLUTION.md).
// v16's one change lives HERE: the JAIL policy is reframed from a defensive cower
// (stay whenever any RIVAL board is developed) to a HAVEN keyed off the bot's OWN
// board (stay only when *I* hold the developed board rivals must traverse — sit to
// collect rent risk-free; otherwise get out and keep moving). See `jailIsHaven` /
// `jailChoice` below. Everything else is v14 verbatim (trades.ts carries v14's
// phantom-denial gate). Shared infrastructure is imported from the canonical
...
// ---------------------------------------------------------------------------
// Jail (v16 sharpening). v5 stayed in jail whenever ANY board out there was
// developed — a DEFENSIVE cower, blind to whether the bot has anything to gain by
// sitting. v16 reframes jail as a HAVEN keyed off MY OWN board: sit to collect rent
// risk-free only when *I* hold the developed board rivals must traverse (sitting is
// then pure upside — I collect when they land and risk nothing by not moving). When
// I DON'T hold such a board, cowering only forfeits my own development, GO, and
// acquisitions — so get out and keep moving. Hypothesis: jail value comes from the
// bot's OWN board, not from fearing rivals' (echoing the aggression-beats-defense
// meta-lesson — v9's defensive liquidity regressed).
// ---------------------------------------------------------------------------
...
/** Does the bot hold a DEVELOPED board worth sitting in jail to collect from — one
 *  of MY properties with dangerous developed rent? Then jail is a profitable haven:
 *  rivals keep circulating and paying my rent while I risk nothing by not moving. */
function jailIsHaven(state: GameState, pid: string): boolean {
  for (const [posStr, owner] of Object.entries(state.ownership)) {
    if (owner !== pid) continue;
    const pos = Number(posStr);
...
export function jailChoice(
  ...
  const player = state.players.find((p) => p.id === pid);
  const cash = player?.cash ?? 0;
  if (jailIsHaven(state, pid)) {
    return {
      intent: null,
      reason:
        "Staying in jail — I hold a developed board, so sitting collects rent risk-free while rivals circulate; rolling for doubles.",
    };
  }
  if (heldCard !== null) {
    return {
