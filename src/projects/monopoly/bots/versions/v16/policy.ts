// ===========================================================================
// v16 SNAPSHOT — fork of v14 (the phantom-denial-fixed base; see EVOLUTION.md). The
// dispatcher is VERBATIM; v16's only change is in `./valuation.ts` (`jailChoice`):
// the jail policy is reframed from a defensive cower into a HAVEN keyed off the
// bot's OWN developed board — sit to collect rent risk-free only when it holds the
// board rivals must traverse, otherwise get out and keep moving. The Bot/BotDecision
// CONTRACT is imported from `../../decision`, not snapshotted. Exposed as `v16Bot`.
// ===========================================================================
 ...
 *
 *  v16 change (vs the v14 base): the JAIL policy (`jailChoice` in `./valuation.ts`)
 *  is reframed from "stay whenever any rival board is developed" (defensive) to
 *  "stay only when the bot itself holds a developed board worth collecting from"
 *  (a haven) — otherwise leave and keep moving. v14's phantom-denial gate and the
 *  whole trade engine are carried verbatim; only the jail decision changes. */
