// ===========================================================================
// v33 SNAPSHOT — fork of v28 (the champion; see EVOLUTION.md).
// v33 adds POSITION-AWARE DEVELOPMENT: reorder the build walk so monopolies
// that opponents are most likely to land on NEXT get developed first. — fork of v17 (the champion; see EVOLUTION.md).
...
/** v33: How likely is an opponent to land on one of my monopoly properties in
 *  the next few turns? Returns a proximity score (0 = nobody nearby, higher =
 *  opponents are close). For each active opponent, compute the forward dice
 *  distance to any property in the set. Being within one roll (2-12 squares)
 *  is high proximity, weighted by roll probability (peaks at 7). */
function monopolyProximity(
  state: GameState,
  pid: string,
  ...
  let maxProximity = 0;
  for (const opp of activeOpponents(state, pid)) {
    const player = state.players.find((p) => p.id === opp.id);
    if (!player || player.inJail) continue;
    const oppPos = player.position ?? 0;
    for (const pos of positions) {
      let dist = (pos - oppPos + 40) % 40;
      if (dist === 0) dist = 40;
      if (dist >= 2 && dist <= 12) {
        const weight = 1 / (Math.abs(dist - 7) + 1);
        maxProximity = Math.max(maxProximity, weight);
      } else if (dist >= 2 && dist <= 24) {
        maxProximity = Math.max(maxProximity, 0.05);
      }
    }
  }
  return maxProximity;
...
export function planBuild(state: GameState, pid: string): BuildPlan | null {
  ...
  // v33: Build monopolies in proximity order
  const myMonopolies = COLORS_BY_WEIGHT.filter((c) => hasMonopoly(state, c, pid));
  const proximityRanked = myMonopolies
    .map((color) => ({
      color,
      proximity: monopolyProximity(state, pid, groupPositions(color)),
    }))
    .sort((a, b) => {
      if (b.proximity !== a.proximity) return b.proximity - a.proximity;
      return COLORS_BY_WEIGHT.indexOf(a.color) - COLORS_BY_WEIGHT.indexOf(b.color);
    });

  for (const { color } of proximityRanked) {
