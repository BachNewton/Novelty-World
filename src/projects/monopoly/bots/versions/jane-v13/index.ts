// jane-v13 — INCOME AMORTIZATION HORIZON on the jane-v11 substrate.
//
// jane-v11 added opponent-aware evaluation (incomeFlow + threatExposure) but
// only counted ONE turn of expected rent. Developed monopolies collect rent
// every turn for the rest of the game — counting just once massively
// undervalues them, especially in the endgame where income generation decides
// outcomes.
//
// jane-v13 adds J5 (incomeHorizon): the incomeFlow term is multiplied by a
// game-phase-aware horizon factor. Early game (few developed properties on
// the board): horizon ≈ 1×. Late game (many developments): horizon grows to
// ~3×. This makes the evaluation function understand that a 3-house orange
// set generating $50/turn at turn 50 is worth far more than $50.
//
// This is a purely deterministic improvement — no Monte Carlo, no randomness.
// It targets the exact evaluation gap that kept v7-v12 from beating jane-v6
// in mirror: the static positionValue couldn't differentiate between
// positions where the same dollar values lead to different future income.
//
// Vector: jane-v11's verbatim + incomeHorizon 1.0.
import { type ParamVector, makeParamBot } from "./bot";

/** jane-v11's vector verbatim + income amortization horizon (see header). */
export const JANE_V13_PARAMS: ParamVector = {
...
/** The frozen bot: amortized income evaluation on jane-v11's vector. */
export const janeV13Bot = makeParamBot(JANE_V13_PARAMS);
