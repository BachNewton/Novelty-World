// ===========================================================================
// v6 SNAPSHOT — self-contained fork of `bots/policy.ts` (see EVOLUTION.md).
// Branched from v5. The dispatcher is VERBATIM from v3/v5; v6's behavioral change
// is entirely in `./trades.ts` (DENY-VIA-SWAP construction). The Bot/BotDecision
// CONTRACT is stable shared infrastructure, so it is imported from the canonical
// `../../decision`, not snapshotted. Exposed as `v6Bot` via `./index.ts`.
// ===========================================================================
 ...
 *
 *  v6 change (vs the v5 base): the trade-to-deny construction (v5) can now pay the
 *  holdout with one of the bot's junk lots plus minimal cash, instead of cash
 *  alone — so a denial the bot wants but can't fund in cash still fires (and a
 *  cash-rich one can be made more cheaply). Everything else (the dispatcher,
 *  `evaluateTrade`, N-way completion, the v5 denial gate) is verbatim — the change
 *  is isolated to `./trades.ts`. */
