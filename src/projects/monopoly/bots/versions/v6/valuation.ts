// ===========================================================================
// v6 SNAPSHOT — a self-contained copy of the bot's policy (see EVOLUTION.md).
// Branched from v5. v6's behavioral change lives entirely in `./trades.ts`
// (DENY-VIA-SWAP: let trade-to-deny pay the holdout with a junk lot + minimal cash
// instead of cash alone, so the proven one-short denial can fire even when cash is
// short); this valuation copy is VERBATIM from v5/v3, kept here so the version is
// whole and can be run/branched in isolation. Shared, non-strategic infrastructure
// (engine, board geometry) is still imported from the canonical modules — only the
// *policy* is snapshotted.
// ===========================================================================
