// jane-v17 — RIVAL DEPLOYABILITY IN TRADE EVALUATION (J8) on jane-v13 substrate.
//
// jane-v13 is champion (incomeHorizon J5, Elo +5.1). jane-v14/v15/v16 proved
// all positionValue modifications beyond J5 degrade performance.
//
// J8 (rivalDeployability) targets a DIFFERENT decision point: trade evaluation.
// Currently rivalThreatCost prices completing an opponent's monopoly at a flat
// `monopolyBonus(color) × RIVAL_THREAT_FACTOR`. This ignores whether the
// opponent can actually DEVELOP the monopoly post-trade. A cash-starved
// opponent who completes a set is stuck at 2× base rent — much less dangerous
// than a cash-rich opponent who can immediately build to 3 houses.
//
// J8 scales rivalThreatCost by the opponent's post-trade deployability:
//   - 0 house levels affordable → ~0.7× threat (less dangerous than assumed)
//   - 3 house levels affordable → 1.0× threat (neutral, current behavior)
//   - 5 house levels affordable → 1.2× threat (more dangerous than assumed)
//
// This makes the bot correctly price the REAL danger of arming opponents.
// When evaluating whether to trade a lot that completes a rival's set:
//   - If the rival is cash-poor, the threat is overpriced → more willing to trade
//   - If the rival is cash-rich, the threat is underpriced → more reluctant to trade
//
// This is orthogonal to all positionValue terms (J3/J4/J5/J6/J7). It modifies
// the TRADE evaluation pipeline only.
//
// Vector: jane-v13's verbatim + rivalDeployability 1.0.
import { type ParamVector, makeParamBot } from "./bot";

/** jane-v13's vector verbatim + rival deployability in trade evaluation (J8). */
export const JANE_V17_PARAMS: ParamVector = {
...
/** The frozen bot: amortized income + rival deployability on jane-v11's vector. */
export const janeV17Bot = makeParamBot(JANE_V17_PARAMS);
