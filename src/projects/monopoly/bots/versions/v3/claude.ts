// ===========================================================================
// v3 SNAPSHOT — self-contained fork of `bots/claude.ts` (see EVOLUTION.md). The
// dispatcher is verbatim; v3's behavioral change is in `./trades.ts` (N-way /
// multi-short trade construction, on top of v2's rival-threat pricing). The
// Bot/BotDecision CONTRACT is stable shared infrastructure, so it is imported
// from the canonical `../../decision`, not snapshotted. Exposed as `v3Bot` via
// `./index.ts`.
// ===========================================================================
 ...
 *
 *  v2 change (vs the v1 champion): the rival-monopoly threat is PRICED rather
 *  than vetoed (`./trades.ts`), so a "cash for the completer" deal can clear
 *  when the buyer pays enough to outweigh the set it hands over. Everything else
 *  is the v1 dispatcher verbatim.
 *
 *  Known limits (inherited from v1, deliberate): trade construction still
