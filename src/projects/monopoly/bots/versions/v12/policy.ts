// ===========================================================================
// v12 SNAPSHOT — fork of the champion v5 (see EVOLUTION.md). The dispatcher is
// VERBATIM from v5; v12's behavioral change is entirely in `./trades.ts` + the new
// `./mix.ts` (the replay-safe RNG seam: MIX which near-equal trade to propose,
// drawn from `state.rngState`). The Bot/BotDecision CONTRACT is stable shared
// infrastructure — and notably the RNG seam needs NO contract change: `rngState`
// is already in the `GameState` the bot receives (see `./mix.ts`). Imported from
// the canonical `../../decision`, not snapshotted. Exposed as `v12Bot`.
// ===========================================================================
 ...
 *
 *  v12 change (vs the v5 base): trade construction's tie-break is now MIXED — among
 *  trade candidates within a tolerance of the best, which one to propose is drawn
 *  deterministically from `state.rngState` (the replay-safe RNG seam, `./mix.ts`)
 *  instead of v5's fixed color order. The hypothesis is information, not value: an
 *  unpredictable proposer denies a modelling opponent a clean read. Everything else
 *  (this dispatcher, `evaluateTrade`, v5's trade-to-deny construction) is verbatim —
 *  the change is isolated to `./trades.ts` (selection) + `./mix.ts` (the draw). */
