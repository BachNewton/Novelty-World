// ===========================================================================
// v8 SNAPSHOT — self-contained fork of `bots/policy.ts` (see EVOLUTION.md).
// Branched from v5, COUPLING two existing building blocks: v5's TRADE-TO-DENY
// (`./trades.ts`) and v4's mortgage-funded TEMPO (`./valuation.ts` `planBuild`).
// The dispatcher itself is VERBATIM from v3/v5; both changes live in the two
// modules it calls. The Bot/BotDecision CONTRACT is stable shared infrastructure,
// imported from `../../decision`, not snapshotted. Exposed as `v8Bot`.
// ===========================================================================
 ...
 *
 *  v8 change (vs the v5 base): COUPLE denial with tempo. v5's trade-to-deny
 *  (`./trades.ts`) is kept verbatim, and v4's mortgage-funded development
 *  (`./valuation.ts` `planBuild`) is folded back in — so after denying a rival
 *  their set, the bot can mortgage idle back-burner lots to out-develop the field
 *  with the freed leverage. The hypothesis is synergy: denial alone (v5) won and
 *  tempo alone (v4) was win-neutral, but the two together may compound — deny, then
 *  press the advantage faster than rivals can recover. Both changes live in the
 *  called modules; the dispatcher is the v3/v5 code verbatim. */
