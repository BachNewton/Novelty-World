// ===========================================================================
// v24 SNAPSHOT — fork of the champion v17 (see EVOLUTION.md). This dispatcher and
// `./valuation.ts` are carried VERBATIM from v17; v24's only change lives in
// `./trades.ts` — `proposeBestTrade` now ASSEMBLES A MONOPOLY FROM SCRATCH (buys
// the whole of a high-value color it holds none of, off its opponent owners), the
// aggressive asymmetric grab that exploits opponents willing to sell property for
// cash. The Bot contract is imported from `../../decision`, not snapshotted.
// Exposed as `v24Bot`.
// ===========================================================================
 ...
 *
 *  v24 change (vs the v17 base): `./trades.ts`'s `proposeBestTrade` now assembles
 *  a monopoly FROM SCRATCH — buys every lot of a high-value color the bot holds
 *  none of, off its opponent owners, gated to real prizes that keep the bot above
 *  its rent reserve. The dispatcher and `./valuation.ts` are carried verbatim from
 *  v17. */
