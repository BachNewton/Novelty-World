...
- **`must-raise-cash` reuses the same `manage` reducer** (forced branch): the current debtor (`firstNegativePlayer`, possibly off-turn) raises by selling buildings and/or mortgaging — no builds, no un-mortgages — and `settleOrRaise` resumes play once they're back to ≥ 0. `maxRaisableCash` counts building sale value so a debtor with only built property settles instead of busting. `settleOrRaise` re-seeds `turn.manageStaged` empty on every entry, so a settled debtor's staged maps never leak into the next debtor's broadcast view.
